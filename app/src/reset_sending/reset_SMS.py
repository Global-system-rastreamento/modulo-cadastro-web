    ...
    try:
        ...
        if response.status_code == 200:
             ...
             if "SMS successfully sent" in response.text or "SMS queued for delivery" in response.text:
                 logging.info("Sucesso: Requisição de envio de SMS enviada e confirmação encontrada na resposta.")
                 return True
             else:
                 logging.info("Alerta: Requisição enviada (status 200), mas mensagem de sucesso não confirmada na resposta HTML.")
                 # Consideramos sucesso se a requisição foi aceita (200 OK)
                 return True
        else:
            logging.error(f"Falha no envio da requisição de SMS (Status: {response.status_code}).")
            return False
    except requests.exceptions.RequestException as e:
        logging.error(f"Erro na requisição para enviar SMS: {e}")
        if hasattr(e, 'response') and e.response is not None:
            logging.error(f"Status Code: {e.response.status_code}")
            logging.error(f"Response Body: {e.response.text}")
        return False
    except Exception as e:
        logging.error(f"Um erro inesperado ocorreu em send_sms: {e}")
        return False
...
def reset_eseye(msisdn_alvo, comando, session):
    ...
    try:
        ...
        logging.error(f'Houve um erro não tratado ao tentar enviar um reset sms eseye para o telefone {msisdn_alvo}, {e}.')
...
def reset_sms(recipient, message):
    ...
    """
    url = "https://sistema.smsbarato.com.br/cgi-bin/individual2.pl"
    usuario = "globalsystem24h@gmail.com"
    senha = "i146dccw" # Consider moving to .env
    ...
    try:
        ...
        response.raise_for_status()
        logging.info(f"SMS API call to {recipient} successful (status {response.status_code}).")
        return response
    except requests.exceptions.RequestException as e:
        logging.error(f"SMS API call to {recipient} failed: {e}")
        return None
