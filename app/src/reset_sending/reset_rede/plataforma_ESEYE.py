...
# --- Configuração ---
BASE_URL = "https://siam3.eseye.com/"
...
def login(username: str, password: str, session: requests.Session = None) -> requests.Session | None:
    ...
    try:
        logging.info(f"Tentando login como {username}...")
        ...
        logging.info(f"URL final após tentativa de login: {final_url}")

        if expected_final_url_path in final_url and response.status_code == 200:
            logging.info("Login realizado com sucesso!")
            return session, PHPSESSIONID
        else:
            logging.error("Falha no login. URL final ou status code inesperado.")
            logging.error(f"Status Code final: {response.status_code}")
            return None

    except requests.exceptions.RequestException as e:
        logging.error(f"Erro na requisição de login: {e}")
        if hasattr(e, 'response') and e.response is not None:
            logging.error(f"Status Code: {e.response.status_code}")
            logging.error(f"Response Body: {e.response.text}")
        return None
    except Exception as e:
        logging.error(f"Um erro inesperado ocorreu durante o login: {e}")
        return None
...
def get_iccid(session: requests.Session, msisdn: str, sess_id) -> str | None:
    ...
    try:
        ...
        logging.error(f"Erro na requisição para obter ICCID: {e}")
        if hasattr(e, 'response') and e.response is not None:
             logging.error(f"Status Code: {e.response.status_code}")
             logging.error(f"Response Body: {e.response.text}")
        return None
        ...
        logging.error(f"Um erro inesperado ocorreu em get_iccid: {e}")
        return None

def send_ticket_network_reset(session, iccid, msisdn):
    """
    Envia um ticket de redefinição de rede para a plataforma Eseye.
    """
    url = "https://siam3.eseye.com/support/create"
    ...
    headers = {
        ...
        "Content-Type": "application/x-www-form-urlencoded",
        "Host": "siam3.eseye.com",
        "Origin": "https://siam3.eseye.com",
        "Referer": "https://siam3.eseye.com/support/create",
        "Sec-Fetch-Dest": "document",
        "Sec-Fetch-Mode": "navigate",
        "Sec-Fetch-Site": "same-origin",
        "Sec-Fetch-User": "?1",
    ...
    try:
        logging.info(f"Enviando ticket de redefinição de rede para MSISDN: {msisdn}...")
        response = session.post(url, headers=headers, data=data, allow_redirects=False)

        if response.status_code == 302:
            logging.info("Ticket criado com sucesso! Redirecionado para: " + response.headers.get("Location"))
            return True
        else:
            logging.error("Falha ao criar o ticket.")
            logging.error(f"Response: {response.text}")
            return False
    except Exception as e:
        logging.error(f"Erro ao enviar ticket de redefinição de rede: {e}")
        return False

def main(msisdn_alvo, session):
    """
    ...
    try:
        ...
        authenticated_session, PHPSESSIONID = login(USERNAME, PASSWORD, session=session)

        if authenticated_session:
            iccid_encontrado = get_iccid(authenticated_session, msisdn_alvo, PHPSESSIONID)

            if iccid_encontrado:
                success = send_ticket_network_reset(authenticated_session, iccid_encontrado, msisdn_alvo)
                return success
            else:
                logging.error("ICCID não encontrado.")
                return False
        else:
            logging.error('Não foi possível autenticar na plataforma da Eseye.')
            return False
    except Exception as e:
        logging.error(f'Ocorreu um erro não tratado durante a execução do reset de rede para {msisdn_alvo}')

if __name__ == '__main__':
    main('5518920039280', requests.Session())
